...
export async function GET(request: NextRequest) {
  ...
  // Build where clause: only requests from students in the gestor's department
  const where: Record<string, unknown> = {}

  // Build AND filters for student-related conditions
  const andFilters: Record<string, unknown>[] = []

  // Department filter: only students in the gestor's department
  if (gestor.id_departamento) {
    andFilters.push({
      estudante: {
        curso: {
          id_departamento: gestor.id_departamento
        }
      }
    })
  }
  ...
  const solicitacoes = await prisma.solicitacaoOrientacao.findMany({
    ...
    orderBy: [
      { estado: "asc" }, // Pendente first (alphabetically)
  ...
  const solicitacoesFormatadas = solicitacoes.map(s => ({
    ...
    orientador: {
      id_orientador: s.orientador.id_orientador,
      nome: s.orientador.nome_completo,
      especialidade: s.orientador.especialidade,
    },
  ...
  const cursosDoDepartamento = gestor.id_departamento
