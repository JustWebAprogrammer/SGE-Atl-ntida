...
export async function PATCH(
  ...
  // Verify the student belongs to the gestor's department
  if (gestor.id_departamento && solicitacao.estudante.curso.id_departamento !== gestor.id_departamento) {
    return NextResponse.json({ error: "Não autorizado a modificar solicitações de estudantes de outro departamento" }, { status: 403 })
  }
    ...
    if (estado === "Aceite") {
      ...
      if (monografia) {
        await tx.monografia.update({
          ...
          data: { id_orientador: solicitacao.id_orientador }
        })
      }
      ...
      if (monografia && monografia.id_orientador === solicitacao.id_orientador) {
        await tx.monografia.update({
          where: { id_monografia: monografia.id_monografia },
  ...
  try {
    ...
    await logAudit({
      ...
      acao: estado === "Aceite" ? "GESTOR_ACEITAR_SOLICITACAO" : "GESTOR_RECUSAR_SOLICITACAO",
