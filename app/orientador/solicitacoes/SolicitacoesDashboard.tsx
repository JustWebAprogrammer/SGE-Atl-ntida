...
export default function SolicitacoesDashboard() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/orientador/solicitacoes?${params.toString()}`)
      .then(r => r.json())
      .then(data => {
  ...
  async function processarSolicitacao(idSolicitacao: number, estado: "Aceite" | "Recusado") {
    ...
    try {
      const res = await fetch(`/api/orientador/solicitacoes/${idSolicitacao}`, {
        method: "PATCH",
  ...
  return (
    <DashboardLayout
      navItems={navItems}
      title="Solicitações de Orientação"
      subtitle="Gerir pedidos de orientação dos estudantes"
    >
      {/* Stats */}
      <div style={{
        display: "grid",
